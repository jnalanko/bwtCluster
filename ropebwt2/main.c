...
// Initialize seq_nt6_table with this at the start of main
static unsigned const char seq_nt6_table[128] = {
...
int main_ropebwt2(int argc, char *argv[])
{
	...
	for (;;) {
		...
		if (flag & FLAG_LINE) { // read a line
			int dret;
			if (ks_getuntil(ks->f, KS_SEP_LINE, &ks->seq, &dret) < 0) {
                break;
            }
